...
function reduceAjvErrors(errors) {
  ...
  instances.forEach(err => {
    ...
    /* if (flattened.has(err.instancePath)) {
      // In the case of an `if` keyword error these'll be the case usually when something within that schema is failing
      // like an `additionalProperties` or `unevaluatedProperties` error. Instead of surfacing the nonsensical "if must
      // match "else" schema" error to the user we should instead prefer the error that's closer to the instancePath
      // where the problem *actually* lies.
      if (flattened.get(err.instancePath).keyword === 'if') {
        flattened.set(err.instancePath, err);
      } else if (flattened.get(err.instancePath).keyword === 'anyOf') {
        // Much like `if` keywords, this is for the same handling of `anyOf` and the "must match a schema in anyOf"
        // messages that they come with.
        flattened.set(err.instancePath, err);
      }
